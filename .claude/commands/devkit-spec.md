...
1. Load and follow **`sbu2.openspec-router`** (Read `.claude/skills/sbu2.openspec-router/SKILL.md` or vendor path in the wrapper).
...
If you already use SBU2/OpenSpec slash names, **`devkit-sync`** copies `/opsx`, `/opsx-apply`, `/opsx-brainstorm`, etc. into `.claude/commands/` when `runtime_engine=sbu2-ai-kit`. Each file is prefixed to route through the same vendored **`sbu2.*`** skills as native OpenSpec.
