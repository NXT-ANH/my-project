...
3. **List routers** from the Hub after sync: **`setup/routers/*.json`** in the kit (or `.claude/vibe-coding/devkit/routers/` after materialize) — each file has `id`, `description`, `routes_to_workflow` — **only pick ids that exist** on disk.
