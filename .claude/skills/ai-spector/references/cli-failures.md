...
**Forbidden without user approval:** hand-edit graph at scale · glob `docs/srs/**` as primary context · skip `graph merge` · continue generation after **fatal** validate errors (`SCHEMA`, `GRAPH-LOAD`, `NO-CYCLE`).

`[CONFORMANCE]` lines are not failures: `validate`, `index` and `hooks pre-commit` all exit 0 on them and keep the graph. Report them and continue; use `--strict` when you want them treated as errors.
