...
- Slash command / CLI↔Agent handoff design: Cursor rule **`agent-orchestrated-commands`** (file **`.cursor/rules/agent-orchestrated-commands.mdc`** after `devkit-sync`).
...
3. Pick `id` only from real files: **`.cursor/vibe-coding/devkit/routers/*.json`** (after `devkit-sync`) — must match an allowed router id (MCP does not invent ids).
...
**Note:** This file syncs from the Hub to **`.cursor/commands/`** and **`.agents/workflows/`** (same markdown, alongside workflow YAML for Antigravity).
