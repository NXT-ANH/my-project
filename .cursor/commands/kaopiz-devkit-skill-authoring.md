...
**After writing `SKILL.md`:** Cursor rule **`kaopiz-devkit-skill-scan-after-write`** (glob `.cursor/skills/**/SKILL.md`) requires the Agent to run **`skill-authoring scan`** — see **REQUIRED** below.
...
5. **Scan after `SKILL.md` exists:** see **REQUIRED** above. You may combine: **`… skill-authoring prompt -m "…" --scan .cursor/skills/<id>`** if the path is known upfront.

6. **Optional** (only if the user explicitly asks): from the prompt, draft **`SKILL.md`** at a valid path (e.g. `.cursor/skills/<id>/SKILL.md` or Hub staging — repo-dependent).
