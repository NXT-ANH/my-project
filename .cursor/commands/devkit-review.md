...
The **`code-review-excellence`** skill must be available. It is automatically pulled from GitHub during `devkit:sync` (or `kaopiz-devkit sync`). Confirm by checking that the skill exists under `.cursor/skills/code-review-excellence/` or `.claude/skills/code-review-excellence/`.
...
This file syncs to **`.cursor/commands/`** and **`.claude/commands/`** (Claude Code).
