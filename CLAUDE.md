...
**Full router:** `.claude/skills/_skill-router.md` · **User guide:** `WORKFLOW.md` · **Workflow triggers:** `.claude/workflows/README.md` · **MCP parity tests:** `src/core/workflow/route-intent-examples.ts`

## Slash commands (override)

When the user invokes a slash command (`/generate-detail-design`, `/resolve-task`, `/review`, etc.), read `.claude/commands/<name>.md` and activate the skill named there. **Do not** re-route via natural-language priority below.
...
Full runbook: `.claude/skills/ai-spector-graph/references/impact.md`
